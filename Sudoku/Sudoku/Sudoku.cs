...
namespace SudokuPuzzle
{
    public class Sudoku
    {
        ...
        public void Solve()
        {
            ...
            while (!this.IsSolved() && _Change)
            {
                ...
                // look for naked pair in box
                _Change = this.CheckNakedSubsetInBoxPencilMarks(2);
                if (_Change) continue;

                // look for naked pair in lines
                _Change = this.CheckNakedSubsetInLinesPencilMarks(2);
                if (_Change) continue;

                // look for naked pair in box
                _Change = this.CheckNakedSubsetInBoxPencilMarks(3);
                if (_Change) continue;

                // look for naked pair in lines
                _Change = this.CheckNakedSubsetInLinesPencilMarks(3);
                if (_Change) continue;
            }
        }
        ...
        #region Naked Subset in box pencil marks
        public bool CheckNakedSubsetInBoxPencilMarks(int size)
        {
            bool _Change = false;
                ...
                for (int j = 0; j < this._BoxSize; j++)
                {
                    for (int y = (i * this._BoxSize); y < (i * this._BoxSize) + this._BoxSize; y++)
                    {
                        for (int x = (j * this._BoxSize); x < (j * this._BoxSize) + this._BoxSize; x++)
                        {
                            if (this._Solution[y, x].IsEmpty())
                            {
                                if (this._Solution[y, x].PencilMarks.Count <= size && this._Solution[y, x].PencilMarks.Count > 1)
                                {
                                    CellCoordinate[] cells = new CellCoordinate[size];
                                    cells[0] = new CellCoordinate(x, y);
                                    if (this.CheckBoxForNakedSubsetPencilMarks(ref cells))
                                    {
                                        if (this.CleanNakedSubsetInBoxPencilMarks(cells))
                                        {
                                            _Change = true;
                                        }
                                        /*
                                        foreach (CellCoordinate c in cells)
                                        {
                                            if (c != null)
                                            {
                                                Console.Write(string.Format("Box: ({0},{1}) = ", c.X, c.Y));
                                                foreach (int p in (int[])this._Solution[c.Y, c.X].PencilMarks.ToArray(typeof(int)))
                                                {
                                                    Console.Write(p + ",");
                                                }
                                                Console.WriteLine("");
        ...
        public bool CleanNakedSubsetInBoxPencilMarks(CellCoordinate[] cells)
        {
            bool _Change = false;
                    ...
                    if (this._Solution[y, x].IsEmpty())
                    {
                        ...
                        if (_Other)
                        {
                            for (int i = 0; i < cells.Length; i++)
                            {
                                if (cells[i] != null)
                                {
                                    int[] numbers = (int[])this._Solution[cells[i].Y, cells[i].X].PencilMarks.ToArray(typeof(int));
        ...
        private bool CheckBoxForNakedSubsetPencilMarks(ref CellCoordinate[] cells)
        {
            int _Count = 0;
            ...
            for (int y = (boxcoordinate.Y * this._BoxSize); y < (boxcoordinate.Y * this._BoxSize) + this._BoxSize; y++)
            {
                for (int x = (boxcoordinate.X * this._BoxSize); x < (boxcoordinate.X * this._BoxSize) + this._BoxSize; x++)
                {
                    if (this._Solution[y, x].IsEmpty())
                    {
                        if ((x != cells[0].X) || (y != cells[0].Y))
                        {
                            if (this._Solution[y, x].PencilMarks.Count <= cells.Length && this._Solution[y, x].PencilMarks.Count > 1)
                            {
                                if (this._Solution[cells[0].Y, cells[0].X].ContainPencilMarks(this._Solution[y, x].PencilMarks))
                                {
                                    _Count++;
                                    if (_Count > (cells.Length - 1))
                                    {
                                        return false;
                                    }
        ...
        #region Naked Subset in lines pencil marks
        public bool CheckNakedSubsetInLinesPencilMarks(int size)
        {
            bool _Change = false;
                ...
                for (int x = 0; x < this._Size; x++)
                {
                    if (this._Solution[y, x].IsEmpty())
                    {
                        if (this._Solution[y, x].PencilMarks.Count <= size && this._Solution[y, x].PencilMarks.Count > 1)
                        {
                            CellCoordinate[] cells = new CellCoordinate[size];
                            cells[0] = new CellCoordinate(x, y);
                            if (this.CheckHorizontalLineForNakedSubsetPencilMarks(ref cells))
                            {
                                if (this.CleanNakedSubsetInLinesPencilMarks(cells))
                                {
                                    _Change = true;
                                }
                                /*
                                foreach (CellCoordinate c in cells)
                                {
                                    if (c != null) Console.WriteLine(string.Format("Horizontal: ({0},{1})", c.X, c.Y));
                                }
                                Console.WriteLine("");
                                */
                            }
                ...
                for (int y = 0; y < this._Size; y++)
                {
                    if (this._Solution[y, x].IsEmpty())
                    {
                        if (this._Solution[y, x].PencilMarks.Count <= size && this._Solution[y, x].PencilMarks.Count > 1)
                        {
                            CellCoordinate[] cells = new CellCoordinate[size];
                            cells[0] = new CellCoordinate(x, y);
                            if (this.CheckVerticalLineForNakedSubsetPencilMarks(ref cells))
                            {
                                if (this.CleanNakedSubsetInLinesPencilMarks(cells))
                                {
                                    _Change = true;
                                ...
                                foreach (CellCoordinate c in cells)
                                {
                                    if (c != null) Console.WriteLine(string.Format("Vertical: ({0},{1})", c.X, c.Y));
                                }
                                Console.WriteLine("");
                                */
                            }
        ...
        public bool CleanNakedSubsetInLinesPencilMarks(CellCoordinate[] cells)
        {
            bool _Change = false;
        ...
        private bool CheckHorizontalLineForNakedSubsetPencilMarks(ref CellCoordinate[] cells)
        {
            int _Count = 0;
            ...
            for (int x = 0; x < this._Size; x++)
            {
                if (this._Solution[cells[0].Y, x].IsEmpty())
                {
                    if (x != cells[0].X)
                    {
                        if (this._Solution[cells[0].Y, x].PencilMarks.Count <= cells.Length && this._Solution[cells[0].Y, x].PencilMarks.Count > 1)
                        {
                            if (this._Solution[cells[0].Y, cells[0].X].ContainPencilMarks(this._Solution[cells[0].Y, x].PencilMarks))
                            {
                                _Count++;
                                if (_Count > (cells.Length - 1))
                                {
        ...
        private bool CheckVerticalLineForNakedSubsetPencilMarks(ref CellCoordinate[] cells)
        {
            int _Count = 0;
            ...
            for (int y = 0; y < this._Size; y++)
            {
                if (this._Solution[y, cells[0].X].IsEmpty())
                {
                    if (y != cells[0].Y)
                    {
                        if (this._Solution[y, cells[0].X].PencilMarks.Count <= cells.Length && this._Solution[y, cells[0].X].PencilMarks.Count > 1)
                        {
                            if (this._Solution[cells[0].Y, cells[0].X].ContainPencilMarks(this._Solution[y, cells[0].X].PencilMarks))
                            {
                                _Count++;
                                if (_Count > (this._BoxSize - 1))
                                {
        ...
        static void Main(string[] args)
        {
            ...
            s.Initialize(",,,,,,,1,,1,3,6,9,5,8,,,4,,,,,,,,,8,,,,,,6,9,,,7,1,,,,2,8,,,8,6,,,,,,,,3,,,,1,,,,2,6,4,1,,,,3,,7,,,7,6,,,,5,,");//Naked Pair
            //s.Initialize(",,7,,6,1,,,,,,,,,8,,6,,1,,6,,7,4,,,3,,2,,,,3,8,,6,,,,,,,,,,,,,,1,,,5,,,,8,7,3,2,6,,5,6,,2,1,8,5,,4,,5,7,,6,4,9,2,,,");//Hidden Pair
            //s.Initialize("2,4,,,3,,,,1,5,9,,,1,,3,2,,,,,,2,,,,4,3,5,2,1,4,6,8,9,7,4,,,3,8,9,5,1,2,1,8,9,5,7,2,6,4,3,,2,,,9,3,1,,,6,,,,5,1,,,9,9,,,,6,,,3,,");//Naked Triple
    ...
    public class SudokuCell
    {
        ...
        public bool ContainPencilMarks(ArrayList numbers)
        {
            bool result = false;

            int[] pencilmarks = (int[])numbers.ToArray(typeof(int));
            for (int i = 0; i < pencilmarks.Length; i++)
            {
                if (!ContainPencilMark(pencilmarks[i]))
                    return false;
                else
                    result = true;
            }
